 ...
 *  compact：顶栏等窄容器用的仅图标形态（UI 体检报告 2026-09-26 阻断#1/#2）。 */
export function UserBadge({ compact = false }: { compact?: boolean }) {
    ...
    if (compact) {
      return (
        <button type="button" onClick={() => auth.login("login")} title="登录或注册"
          aria-label="登录或注册"
          className="grid size-8 shrink-0 place-items-center rounded-full border border-bd-card bg-card text-ink shadow-sm transition-shadow hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">
  ...
  return (
    <>
      {compact ? (
        <button type="button" onClick={() => setOpen(true)} aria-label="打开个人中心" title={name}
          className="grid size-8 shrink-0 place-items-center rounded-full bg-accent-soft text-body-sm font-black text-accent-ink transition-transform hover:scale-105 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">
          {name[0]?.toUpperCase() ?? "?"}
        </button>
      ) : (
